...
// ── Hero: Insert podcast card + explore topics ────────────────────────────────
// The file uses CRLF (\r\n) line endings
const HERO_FIND = '</section>\r\n\r\n          {/* Quick Information Trust Anchors */}';
if (!app.includes(HERO_FIND)) { console.error('HERO_FIND not found'); process.exit(1); }

const PODCAST_CARD = [
  '{/* Mobile-only mini podcast card */}',
  '<div className="mob-podcast-card" onClick={() => setPage(\'podcast\')}>',
  '  <div className="mob-podcast-card-inner">',
  '    <div className="mob-podcast-ep-badge"><span style={{ fontSize:\'20px\' }}>{PODCAST_EPISODES[0].coverEmoji}</span></div>',
  ...
  '    <div className="mob-podcast-bar"><div className="mob-podcast-fill" style={{ width: currentEpisode.id === PODCAST_EPISODES[0].id ? `${podcastProgress}%` : \'0%\' }} /></div>',
  ...
  '    {[',
  '      { id:\'hiv\', emoji:\'🛡️\', label:\'HIV\', sub:\'Prevention & support\', color:\'#059669\', bg:\'#D1FAE5\' },',
  '      { id:\'pregnancy\', emoji:\'🌸\', label:\'Pregnancy\', sub:\'Family planning\', color:\'#DB2777\', bg:\'#FCE7F3\' },',
  '      { id:\'compare\', emoji:\'⚖️\', label:\'Compare\', sub:\'Methods & options\', color:\'#D97706\', bg:\'#FEF3C7\' },',
  '      { id:\'directory\', emoji:\'📍\', label:\'Services\', sub:\'Nearby clinics\', color:\'#0891B2\', bg:\'#E0F2FE\' },',
  '      { id:\'podcast\', emoji:\'🎙️\', label:\'Podcast\', sub:\'Real stories\', color:\'#7C3AED\', bg:\'#EDE9FE\' },',
  '    ].map(item => (',
...
const HERO_REPL = '</section>\r\n\r\n' + PODCAST_CARD + '\r\n\r\n          {/* Quick Information Trust Anchors */}';
app = app.replace(HERO_FIND, HERO_REPL);
console.log('✓ Hero mobile sections added');

// ── CSS ───────────────────────────────────────────────────────────────────────
const CSS_OLD = '/* Responsive adjust */\n@media(max-width: 768px) {\n  .hero-banner {\n    padding: 120px 16px 40px;\n  }\n  .hero-title {\n    font-size: 34px;\n  }\n  .nav-header {\n    padding: 12px 16px;\n  }\n  .logo-title {\n    font-size: 20px;\n  }\n  .nav-button span {\n    display: none;\n  }\n  .nav-button {\n    padding: 8px;\n  }\n}\n\n.hide-scrollbar::-webkit-scrollbar {\n  display: none;\n}\n.hide-scrollbar {\n  -ms-overflow-style: none;\n  scrollbar-width: none;\n}';

if (!css.includes(CSS_OLD)) { console.error('CSS_OLD not found'); process.exit(1); }

const CSS_NEW = `/* Desktop: hide mobile-only elements */
...
console.log('✓ Files written — now run build to verify');
